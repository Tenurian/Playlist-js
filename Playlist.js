var Playlist = function (data) {
    ...
    var playNext = function () {
        ...
        for (x = 0; x < tracks.length; x++) {
            var xtrack = tracks[x];
            if (!showAll) {
                if (xtrack.id.match(/\d/g) == SongList[currentSong]) {
    ...
    if (PlaylistElement != null) {
        if (links) {
            if (links.length == 0) {
                ...
                content += "<style>.hidden{visibility: collapse; display:none}.right{float:right}.song{width:" + ((data.hasOwnProperty('width')) ? data.width : "500px") + "}.player{padding-left:5px;padding-right:5px}</style>";
                ...
                for (c = 0; c < tracks.length; c++) { //hehe
                    ...
                    tk.addEventListener('timeupdate', function () {
                        ...
                        document.getElementById('tracktime-' + (this.id.match(/\d/g))).innerHTML = (((min > 0) ? min : "0") + ":" + ((sec >= 10) ? Math.floor(sec) : "0" + Math.floor(sec)));
